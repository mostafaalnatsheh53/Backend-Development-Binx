# Phase 3 Capstone - Cardiac Patient Monitoring

## Initial Project Skeleton and Handoff

This Day 5 project is the starting skeleton for the Phase 3 capstone. It is a healthcare management API for patients, vital signs, medications, appointments, and authentication.

### Current Baseline

- ASP.NET Core Web API with controllers and Swagger/OpenAPI
- Entity Framework Core with SQL Server and an initial migration
- ASP.NET Core Identity and JWT authentication
- Centralized exception handling with safe ProblemDetails responses
- xUnit and Moq test project referencing the API project
- Risk score business logic with unit tests

## Week 5 Summary - Mentor Check-in

### Chosen Project and Scope

The chosen Phase 3 capstone is **Cardiac Patient Monitoring**, a healthcare management backend. The initial scope includes user registration and login, patient records, vital signs, medications, appointments, SQL Server persistence, JWT authentication, Swagger/OpenAPI, and centralized error handling.

The existing Week 1-4 project was reused as the starting skeleton. It already contains the core entities, controllers, services, EF Core migration, authentication setup, and test project needed for Sprint 1.

### Test Suite

The test suite uses xUnit and Moq. The three highest-risk logic areas identified were:

1. `RiskScoreCalculator`: validates vital inputs and combines multiple risk penalties.
2. `AuthService`: validates credentials, handles registration failures, and creates JWT claims.
3. `VitalSignService`: validates patient readings and prevents invalid blood-pressure or future timestamp data.

Unit tests cover the success and failure branches for each area. Integration tests use `WebApplicationFactory` and EF Core InMemory to exercise `GET /api/patients` through HTTP:

- A valid JWT returns `200 OK` with seeded patient data.
- A request without a JWT returns `401 Unauthorized`.

### Error Handling Setup

`ExceptionHandlingMiddleware` is registered centrally before authentication, authorization, and controllers. It maps known application errors to consistent `ProblemDetails` responses:
...
Unexpected errors are logged server-side without exposing exception messages, stack traces, database details, or connection strings to API clients.

### Verification

The full suite was run from the Day5 project directory with:

```powershell
dotnet test
```

Result: **30 tests passed, 0 failed, 0 skipped**. The solution builds successfully and is ready for Sprint 1 mentor review.
...
Sprint 1 should take this baseline through a working, documented set of core routes. Each new endpoint or business rule should include validation, centralized error behavior, and focused tests.

Run the baseline checks from this directory:
...
The next sprint can extend the existing schema and routes without creating a second project or duplicating the Week 1-4 work.
