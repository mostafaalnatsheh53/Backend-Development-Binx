...
[Authorize]
[Route("api/orders")]
public class OrdersController(IOrderService s) : ControllerBase
{
    [HttpPost]
    [ProducesResponseType<OrderResponseDto>(201)]
    [ProducesResponseType(400)]
    [ProducesResponseType(404)]
    public async Task<ActionResult<OrderResponseDto>> Create(
        CreateOrderRequestDto d)
    {
        var order = await s.CreateAsync(d);
