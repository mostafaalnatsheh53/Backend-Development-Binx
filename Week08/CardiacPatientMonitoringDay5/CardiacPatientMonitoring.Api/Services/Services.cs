    ...
    IConfiguration config) : IAuthService
{
    public async Task<AuthResponseDto> RegisterAsync(RegisterDto d)
    {
        ...
        var roleResult = await users.AddToRoleAsync(u, "Patient");
        ...
        return await TokenAsync(u, patient.Id);
    }
    ...
    public async Task<AuthResponseDto> LoginAsync(LoginDto d)
    {
        ...
        var patient = await db.Patients
            .AsNoTracking()
            .SingleOrDefaultAsync(p => p.UserId == u.Id);
    ...
    private async Task<AuthResponseDto> TokenAsync(ApplicationUser u, int patientId)
    {
        var j = config.GetSection("Jwt");
        var expirationMinutes = j.GetValue<int?>("ExpirationMinutes")
            ?? throw new InvalidOperationException(
                "JWT expiration is not configured.");

        if (expirationMinutes <= 0)
            throw new InvalidOperationException("JWT expiration must be positive.");

        var expires = DateTime.UtcNow.AddMinutes(expirationMinutes);
        var roles = await users.GetRolesAsync(u) ?? Array.Empty<string>();
    ...
    private static void ValidatePatientDetails(RegisterDto d)
    {
...
{
    public const string PatientsKey = "catalog:patients:list:v1";
}

public class PatientService(
    ApplicationDbContext db,
    IDistributedCache cache,
    IConfiguration configuration) : IPatientService
{
    ...
    public async Task<IEnumerable<PatientResponseDto>> GetAllAsync(string? s)
    {
        ...
        {
            return await QueryAsync(s);
        }

        var cached = await cache.GetStringAsync(CatalogCache.PatientsKey);
        if (cached is not null)
        {
            return JsonSerializer.Deserialize<List<PatientResponseDto>>(cached)
                ?? [];
        }

        var patients = await QueryAsync(null);
        var expirationMinutes = configuration.GetValue<int?>(
            "CacheSettings:CatalogExpirationMinutes") ?? 5;

        await cache.SetStringAsync(
            CatalogCache.PatientsKey,
    ...
    private async Task<List<PatientResponseDto>> QueryAsync(string? search) =>
        await db.Patients
            .AsNoTracking()
            .Where(p =>
    ...
    private Task InvalidateCatalogAsync() =>
        cache.RemoveAsync(CatalogCache.PatientsKey);
